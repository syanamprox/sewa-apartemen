    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ action('PenghuniController@store') }}" method="post" enctype="multipart/form-data">
                    @csrf
...
<div class="modal fade" id="editData" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ url('master/penghuni') }}" method="post" enctype="multipart/form-data" id="editForm">
                    @method('PATCH')
                    @csrf
                    <div class="form-group">
                        <label>Name</label>
...
<div class="modal fade" id="deleteData" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ url('master/penghuni') }}" method="post" id="deleteForm">
                    @method('DELETE')
                    @csrf
        ...
        <div class="widget-content widget-content-area">
            <div class="table-responsive">
                ...
                <table class="table table-bordered table-hover mb-4" id="myTable">
                    ...
                    <tbody>
                    @foreach($penghuni as $pgn)
                        <tr>
                            <td>{{ $pgn->name }}</td>
                            <td>{{ $pgn->email }}</td>
                            <td>{{ $pgn->no_ktp }}</td>
                            <td>{{ $pgn->tanggal_lahir }}</td>
                            <td>{{ $pgn->alamat }}</td>
                            <td>{{ $pgn->telepon }}</td>
                            <td>{{ $pgn->gaji }}</td>
                            <td>{{ $pgn->getImageAdmin() }}</td>
                            <td class="text-center">
                                <div class="dropdown custom-dropdown">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuLink1">
                                        <a href="{{ url('/master/penghuni/'.$pgn->_id) }}" class="dropdown-item">Show</a> 
                                        <button type="button" class="dropdown-item edit" data-toggle="modal" data-target="#editData" data-id="{{ $pgn->_id }}" data-name="{{ $pgn->name }}" data-email="{{ $pgn->email }}" data-noktp="{{ $pgn->no_ktp }}" data-ttl="{{ $pgn->tanggal_lahir }}" data-alamat="{{ $pgn->alamat }}" data-telepon="{{ $pgn->telepon }}" data-gaji="{{ $pgn->gaji }}" data-image="{{ $pgn->getImageAdmin() }}">Edit</button>
                                        <button type="button" class="dropdown-item delete" data-toggle="modal" data-target="#deleteData" data-id="{{ $pgn->_id }}">Delete</button>
                                    </div>
                                </div>
                            </td>
                        </tr>
...
$(document).ready( function () {
    ...
    $(document).on('click', '.edit', function () {
        ...
        $('#editForm').attr('action','/master/penghuni/'+id);
    });
    ...
    $(document).on('click', '.delete', function () {
        ...
        $('#deleteForm').attr('action','/master/penghuni/'+id);
    });
